...
from utils.constants import ModelNameConst, PodcastPromptConst
...
if __name__ == "__main__":
    ...
    gpt_system_prompt = [{"role" : "system",
                        "content" : format_prompt(PodcastPromptConst.CONTESTANT_ACTIVATION_PROMPT)}]

    gemini_system_prompt = [{"role" : "system",
                            "content" : format_prompt(PodcastPromptConst.CONTESTANT_ACTIVATION_PROMPT)}]

    judge_system_prompt = [{"role" : "system",
                            "content" : format_prompt(PodcastPromptConst.JUDGE_ACTIVATION_PROMPT)}]

    input_prompts = [gpt_system_prompt,
                    gemini_system_prompt,
                    judge_system_prompt]

    first_contestant = ModelNameConst.UPGRADED_GPT_MODEL_NAME
    second_contestant = ModelNameConst.GEMINI_PAID_MODEL_NAME
    competition_judge = ModelNameConst.GPT_MODEL_NAME

    candidate_df = pd.DataFrame({"first_contestant" : [first_contestant],
                                 "second_contestant" : [second_contestant],
                                 "judge" : [competition_judge],
                                 "provocation_prompt": [PodcastPromptConst.PROVOCATION_PROMPT],
                                 "scoring_prompt": [PodcastPromptConst.JUDGE_PROMPT]})

    candidate_df.to_csv(PathConst.BATTLE_CANDIATES)

    gpt_output = gemini_output = PodcastPromptConst.USER_PROMPT + \
                                 " " + PodcastPromptConst.INITIAL_PROMPT_PART

    acc_score_gpt = acc_score_gemini = 0
    toss_and_pass = np.random.randint(2, size=1)[0]

    if toss_and_pass == 0:
        gpt_output = gpt_output + " " + PodcastPromptConst.POSITIVE_SIDE_PROMPT
        gemini_output = gemini_output + " " + PodcastPromptConst.NEGATIVE_SIDE_PROMPT

    else:
        gpt_output = gpt_output + " " + PodcastPromptConst.NEGATIVE_SIDE_PROMPT
        gemini_output = gemini_output + " " + PodcastPromptConst.POSITIVE_SIDE_PROMPT
